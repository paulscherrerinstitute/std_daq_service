...
def tune(machine_config, interfaces, start_udp_port):
    _logger.info(f'Tuning interfaces {interfaces} with start_udp_port {start_udp_port}.')
    ...
    for interface in interfaces:
        ...
        for module_id, cpu_id in map_module_to_cpu.items():
            ...
            udp_port = start_udp_port + module_id
            cmd = ["ethtool", "-U", interface, "flow-type", "udp4", "dst-port", udp_port, "action", module_id]
...
def main():
    ...
    for machine_config in deployment_config:
        ...
        if is_hostname_from_this_machine(machine_config['hosts']):
            tune(machine_config, ['ens2f', 'ens5f'], int(daq_config['start_udp_port']))
            break
    else:
